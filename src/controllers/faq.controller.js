...
exports.updateFaqItem = async (req, res) => {
  try {
    ...
    if (!faqList || !faqList.faqs[index]) {
      return res.status(404).json({ message: 'FAQQ item not found' });
    }
...
exports.updateFaqList = async (req, res) => {
  try {
    const { faqs } = req.body;

    if (!Array.isArray(faqs)) {
      return res.status(400).json({ message: 'Invalid data format. Expected an array of FAQs.' });
    }

    const faqList = await Faq.findOne();
    if (!faqList) {
      return res.status(404).json({ message: 'FAQ list not found' });
    }

    faqList.faqs = faqs; // Replace entire array
    await faqList.save();

    res.status(200).json({ message: 'FAQ list updated successfully', faqs: faqList.faqs });
  } catch (err) {
    res.status(500).json({ message: 'Error updating FAQ list', error: err.message });
  }
};
